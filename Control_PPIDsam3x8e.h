...
// ROLL CONTROL P-PID  control  By tinnakon///////////
  float setpoint_roll = ((CH_AILf-CH_AIL_Cal)*0.085f);//0.12 max +-45 deg  ////+-18  + Control_YBf
  applyDeadband(setpoint_roll, 2.5f);//2.5 ,1.2
  if(Mode == 2 || Mode == 3 && GPS_FIX  == 1){
    setpoint_roll = Control_YBf;
  }
  setpoint_rate_roll = (0.095f*setpoint_rate_roll/(0.095f+G_Dt)) + ((setpoint_roll-setpoint_rollold)/(0.095f+G_Dt));//0.065 ,Diff remote
  setpoint_rollold = setpoint_roll;
  setpoint_rate_roll = constrain(setpoint_rate_roll, -80.0f, 80.0f);//+-80 deg/s
  ...
  roll_I_rate = constrain(roll_I_rate, -5.0f, 5.0f);//5 ,+-150
  ...
  u_roll = constrain(u_roll, -280.0f, 280.0f);//+-250 +-300 120
// PITCH CONTROL P-PID  control  By tinnakon///////////
  float setpoint_pitch = ((CH_ELEf-CH_ELE_Cal)*-0.085f);//max +-45 deg  ////+-18 - Control_XBf
  applyDeadband(setpoint_pitch, 2.5f);//1.2
    if(Mode == 2 || Mode == 3 && GPS_FIX  == 1){
    setpoint_pitch = Control_XBf;
  }
  setpoint_rate_pitch = (0.095f*setpoint_rate_pitch/(0.095f+G_Dt)) + ((setpoint_pitch-setpoint_pitchold)/(0.095f+G_Dt));//0.065 Diff remote
  setpoint_pitchold = setpoint_pitch;
  setpoint_rate_pitch = constrain(setpoint_rate_pitch, -80.0f, 80.0f);//+-80 deg/s
  ...
  pitch_I_rate = constrain(pitch_I_rate, -5.0f, 5.0f);//+-150
  ...
  u_pitch = constrain(u_pitch, -280.0f, 280.0f);//+-250 +-300 120
// YAW  CONTROL P-PID  control  By tinnakon///////////
float setpoint_rate_yaw = (CH_RUDf-CH_RUD_Cal)*0.42f;//0.55 0.4 0.35
  applyDeadband(setpoint_rate_yaw, 6.3f);//6.5
  if(abs(setpoint_rate_yaw) > 0.1f){
   setHeading = ahrs_y*RAD_TO_DEG;// 0 degree ,ahrs_tin.h
  }
  float error_yaw = 0.0f - Heading;
  float error_rate_yaw = setpoint_rate_yaw + error_yaw*Kp_levelyaw - GyroZf*RAD_TO_DEG;
  yaw_I_rate += error_rate_yaw*Ki_rateYaw*G_Dt;
  yaw_I_rate = constrain(yaw_I_rate, -120.0f, 120.0f);//+-120
  ...
  u_yaw = constrain(u_yaw, -360.0f, 360.0f);//+-340 +-170 +-150
 ////Altitude//////////////////////////////////////////////////////////////////////////////////////////////////////  
 Velocity_THR = (CH_THRf - 1500.0f)*0.00369f;//0.00469 +- 2.1 m/s
 applyDeadband(Velocity_THR, 0.23f);//0.23 0.17
  if(Mode == 1 || Mode == 2)//Altitude Hold, 
  {
   ...
   // float error_hII = err_hz*1.25 + error_Vz;//0.55
    ...
    //state feedback + PID + adaptive controller + Kalman filter //https://li01.tci-thaijo.org/index.php/rmutsb-sci/article/view/149110/116850
...
uAltitude = (I_throttle + uthrottle)/cos_rollcos_pitch;//m*g = 10.8 N = 
...
void Control_PositionHold(){
  if(Mode == 2 || Mode == 3 && GPS_FIX  == 1){
          float targetRB_speedLAT = ((CH_ELEf-CH_ELE_Cal)*0.624f);//vX Body Frame ,,0.444 = 200 cm/s
          float targetRB_speedLON = ((CH_AILf-CH_AIL_Cal)*0.624f);//vY Body Frame
