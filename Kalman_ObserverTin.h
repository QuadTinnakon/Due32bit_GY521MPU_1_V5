...
https://www.facebook.com/tinnakonza
 ...
 A=[0 1 0;0 0 1;0 0 -0.003084745763]; 
 B=[0;0;0.84745763];  
 C = [1 1 1];
 Plant = ss(A,B,C,0,0.005); //SYS = SS(A,B,C,D,Ts) creates a discrete-tim
 Q = 0.015; % A number greater than zero
 R = 0.1853; % A number greater than zero
...
///Y/////////////////////////////////////////////////////////////////////////////////
      ...
      double er_xt = (GPS_coordLAT*1e-7 - GPS_LAT_HOMEe)*cm_per_deg_lat - x1_xt;//R = 6,367,449 m; // Radius of earth in KM = 109397.6142 xx
      float er_vt = velocity_north - x2_vt;//cm/s
      float er_at = accrX_Earth*100.0f - x3_at;//cm/s^2
      double ey_xt = (GPS_coordLON*1e-7 - GPS_LON_HOMEe)*cm_per_deg_lon - y1_xt;//R = 6,367,449 m; // Radius of earth in KM = 109397.6142 xx
      ...
      x1_hat = GPS_LAT_HOMEe + (vx1_hat/cm_per_deg_lat);
//Y/////////////////////////////////////////////////////////////////////////////
      ...
      y1_hat = GPS_LON_HOMEe + (vy1_hat/cm_per_deg_lon);
...
z3_hat = z3_acc + 0.099962f*ee3;//0.088824 0.018824 ,0.18824 m/s^2 ,,0.12824 0.21845
//K3 = 0.00124 0.12502 ,, k3 = 0.01641023 ,, k-3 = 0.12367 0.72367
z2_hat = z2_hat2 + 0.0008061f*ee2;//0.001381202 m/s 0.000981202f, 0.00151202 0.00751202 0.086056
//K2 = 0.0091202 0.012502 0.0002141023, 0.0001641, 0.00145, 0.00045,,,k2 = 0.0001641023 0.097502,  0.065502, 0.009
z1_hat = z1_hat2 + 0.0033184f*ee1;//0.0043184 0.0061102  0.0081102 0.011102 0.015102 0.035102, 0.015102 0.065 0.2887 ,, k1 = 0.0001741023 0.09187 0.01187 0.0140
}
