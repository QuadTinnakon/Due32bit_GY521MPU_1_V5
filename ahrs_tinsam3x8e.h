...
float Heading = 0.0f;
float setHeading = 0.0f;
float previousEx = 0.0f;
float previousEy = 0.0f;
float previousEz = 0.0f;
...
boolean isSwitched(float previousError, float currentError) {
  if ((previousError > 0.0f &&  currentError < 0.0f) || (previousError < 0.0f &&  currentError > 0.0f)) {
    return true;
  }
  return false;
}
void ahrs_updateMARG(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float G_Dt){
  ...
  if(cos_rollcos_pitch <= 0.866025403f)//30 deg = 0.866025403
    {
      cos_rollcos_pitch = 0.866025403f;
    }
////transition between 0 and 360 or -180 and 180 ,By aeroquad//////////    
     Heading = ahrs_y*RAD_TO_DEG - setHeading;
    if (ahrs_y*RAD_TO_DEG <= (setHeading - 180.0f)){
      Heading += 360.0f;
    }
    if (ahrs_y*RAD_TO_DEG >= (setHeading + 180.0f)){
      Heading -= 360.0f;
...
gyroX_Earth = gyroX_Earth + (gyroX_Earthff - gyroX_Earth)*G_Dt*12.51f;//32.51
gyroY_Earth = gyroY_Earth + (gyroY_Earthff - gyroY_Earth)*G_Dt*12.51f;
gyroZ_Earth = gyroZ_Earth + (gyroZ_Earthff - gyroZ_Earth)*G_Dt*12.51f;
}
